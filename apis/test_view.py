...
class GetAllPuppiesTest(TestCase):
    ...
    def setUp(self):
        TaskTrack.objects.create(
            name='Task1', description='Testing models', date=timezone.now, state='to_do', priority='0_low')
        TaskTrack.objects.create(
            name='Some Sample', description='Just smth', date=timezone.now, state='to_do', priority='1_med')
        TaskTrack.objects.create(
...
class GetSinglePuppyTest(TestCase):
    ...
    def test_get_valid_single_puppy(self):
        response = client.get(
            reverse('get_delete_update_task', kwargs={'pk': self.rambo.pk}))
        task = TaskTrack.objects.get(pk=self.rambo.pk)
        serializer = serializers.ListTaskSerializer(task)
        self.assertEqual(response.data, serializer.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
